...
const techEventsList = eventsList.filter((event) =>
          ...
          >
            <div className="p-6 grow flex flex-col">
              <div className="flex justify-between items-start mb-4">
                <h2 className="text-3xl text-[#FFD700] font-wizard">
                  {event.title}
                </h2>
                <span
                  className={`text-xs font-bold px-2 py-1 rounded border backdrop-blur-sm ${tagColors[event.tag] || "text-gray-300 border-gray-500"}`}
                >
                  {event.tag}
                </span>
              </div>
              <p className="text-sm text-gray-300 mb-6 font-sans leading-relaxed grow">
                {event.shortDesc}
              </p>
